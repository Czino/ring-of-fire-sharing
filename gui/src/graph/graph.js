...
class Graph {
  ...
  drawArrow(x1, y1, x2, y2, text = "", directional = false, opacity = 1) {
  ...
  check_mouse() {
    ...
    function check(e) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        if (nodes[i].inside(this.mousex, this.mousey)) {
          ...
          this.active = nodes[i];
          this.active.onMouseEnter(this.active, this.mousex, this.mousey)
          return;
        }
      }
      for (let edge of Object.values(this.edges)) {
      ...
      if (this.active && this.active.onMouseLeave) this.active.onMouseLeave(this.active, this.mousex, this.mousey)
    ...
    function onClick() {
      ...
      this.active.onClick(this.active, this.mousex, this.mousey)
    }
  ...
  edge(startNodeid, endNodeid, color = "#aaa", text = "", directional = false, width = 1) {
    return new Graph._edge(this.id, startNodeid, endNodeid, color = color, text = text, directional = directional, width = width);
  }
  node(x = false, y = false, r = false, text = "", width = 1) {
    let node = new Graph._node(this.id, x = x, y = y, r = r, text = text, width = width);
    this.nodeSetupCallback(node);
    return node;
  }
}

Graph._edge = function (contextid, startNodeid, endNodeid, color = "#000", text = "", directional = false, width = 1) {
  this.type = "edge";
  ...
  this.draw = function () {
    ...
    context.ctx.lineWidth = this.width || 1
    ...
    if (this.startNodeid == this.endNodeid) {
      ...
      context.drawArrow(this.xstart, this.ystart, this.xend, this.yend, this.text, this.directional, this.opacity ?? 1);
    }
...
Graph._node = function (contextid, x = false, y = false, r = false, text = "", width = 1) {
  this.type = "node"
  this.children = [];
  this.edges = {};
  this.contextid = contextid;
  ...
  this.create = function (x, y, r, text, width) {
    this.x = x;
    this.y = y;
    this.r = r;
    this.color = '#000';
  ...
  this.build = function () {
    ...
    context.ctx.globalAlpha = this.opacity ?? 1;
    if (this.active) {
      context.ctx.lineWidth = this.width || 10;
    } else {
      context.ctx.lineWidth = this.width || 1;
    }
    context.ctx.strokeStyle = this.color;
  ...
  this.connect = function (n, text = "", directional = false, width = 1) {
    let context = Graph.getContext(this.contextid);
    if (n.type !== "node") {
      return false;
    }
    ...
    let edge = context.edge(this.id, n.id, this.color, text = text, directional = directional, width = width);
    this.edges[edge.id]
